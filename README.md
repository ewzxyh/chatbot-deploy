...
The Worker validates HMAC signed URLs generated by the backend and then reads the object through an R2 binding. Production should use `MEDIA_CDN_REPLACE_SRC=true` after the Worker is validated: new media records prefer signed CDN URLs while preserving `/api/files` proxy URLs as fallback metadata.
